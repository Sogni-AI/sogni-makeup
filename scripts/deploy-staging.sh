...
# Comprehensive staging deployment script for Sogni Makeover
...
echo "🚀 Starting Sogni Makeover Staging Deployment"
echo "=================================================="

# Configuration
REMOTE_HOST="sogni-staging"
REMOTE_FRONTEND_PATH="/var/www/makeover-staging.sogni.ai/dist"
REMOTE_BACKEND_PATH="/var/www/makeover-staging.sogni.ai/backend"
...
ssh $REMOTE_HOST "sudo mkdir -p /var/www/makeover-staging.sogni.ai/dist /var/www/makeover-staging.sogni.ai/backend && sudo chown -R \$USER:\$USER /var/www/makeover-staging.sogni.ai"
...
rsync -ar --progress scripts/nginx/staging.conf $REMOTE_HOST:/tmp/sogni-makeover-staging.conf
if [ $? -ne 0 ]; then
...
ssh $REMOTE_HOST << EOF
  ...
  pm2 delete sogni-makeover-staging 2>/dev/null || true
  PORT=3003 pm2 start index.js --name sogni-makeover-staging
  pm2 save
  ...
  pm2 status sogni-makeover-staging
  
  # Configure nginx
  echo "🔍 Setting up nginx configuration..."
  sudo mkdir -p /etc/nginx/conf.d/
  sudo cp /tmp/sogni-makeover-staging.conf /etc/nginx/conf.d/makeover-staging.sogni.ai.conf
...
echo "🔍 Checking backend health via branded domain (https://makeover-api-staging.sogni.ai)..."
HEALTH_CHECK=$(ssh $REMOTE_HOST "curl -s -o /dev/null -w '%{http_code}' https://makeover-api-staging.sogni.ai/health" || echo "failed")
if [ "$HEALTH_CHECK" = "200" ]; then
  ...
  echo "⚠️ Warning: The backend may not be running correctly. Please check logs with: ssh $REMOTE_HOST 'pm2 logs sogni-makeover-staging'"
fi

echo "🔍 Checking frontend availability (https://makeover-staging.sogni.ai)..."
FRONTEND_CHECK=$(curl -s -o /dev/null -w "%{http_code}" -I https://makeover-staging.sogni.ai/ || echo "failed")
...
echo "Frontend: http://makeover-staging.sogni.ai/"
echo "Backend API: http://makeover-api-staging.sogni.ai/"
if [ -f server/.env.staging ]; then
