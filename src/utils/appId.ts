 ...
 * This appId represents the Makeover app installation, not the user
...
function getAppIdKey(): string {
  ...
  if (hostname === 'makeover.sogni.ai') {
    return 'sogni-makeover-app-id-production';
  } else if (hostname.includes('staging')) {
    return 'sogni-makeover-app-id-staging';
  } else if (hostname === 'makeover-local.sogni.ai') {
    return 'sogni-makeover-app-id-local';
  } else {
    // localhost or other development
    return 'sogni-makeover-app-id-dev';
  }
}
