...
const CANCEL_CONFIRMATION_COOKIE = 'sogni_makeover_cancel_no_confirm';

// Local storage key for tracking last cancel time (backup for rate limiting)
const LAST_CANCEL_TIME_KEY = 'sogni_makeover_last_cancel_time';
...
export function notifyCancelStateChange(): void {
  ...
  if (!state.canCancel && state.cooldownRemaining > 0) {
    ...
    // Safety cleanup after rate limit window expires
    setTimeout(() => clearInterval(checkInterval), CANCEL_RATE_LIMIT_MS + 1000);
  }
}
