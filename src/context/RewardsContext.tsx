...
export const RewardsProvider: React.FC<RewardsProviderProps> = ({ children }) => {
  ...
  const fetchRewards = useCallback(async () => {
    ...
    if (isFetchingRef.current) {
      console.log('Skipping duplicate rewards fetch request');
      return;
    }
    ...
    try {
      ...
      if (error.message?.includes('429') || error.statusCode === 429) {
        ...
        console.warn(`Rate limited (429). Backing off for ${backoffMinutes} minutes`);
  ...
  useEffect(() => {
    if (claimIntent.id && claimIntent.token) {
      ...
      sogniClient.account
        ...
        .then(() => {
          ...
          showToast(`Reward Claimed! +${totalAmount} credits`, 'success');

          // Play sonic logo for reward claim (always enabled in makeover)
        ...
        .catch((err: unknown) => {
          ...
          if (error.message?.includes('429') || error.statusCode === 429) {
            ...
            rateLimitBackoffRef.current = Date.now() + (backoffMinutes * 60 * 1000);
            console.warn(`Rate limited (429) on claim. Backing off for ${backoffMinutes} minutes`);
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [claimIntent]); // Only trigger when claimIntent changes
  ...
  const contextValue = useMemo<RewardsContextType>(
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps
    [rewards, error, loading, claimLoading, claimInProgress, lastClaimSuccess]
  );
